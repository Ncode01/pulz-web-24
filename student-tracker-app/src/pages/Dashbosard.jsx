...
export default function Dashboard({ setPage }) {
  ...
  const handlePredict = (subjects) => {
    const floatMarks = subjects.map((s) => parseFloat(s) || 0);
    const tensor = tf.tensor(floatMarks);
    // Example: average 9 marks, add a small offset
    const score = tensor.mean().dataSync()[0] + 5;
          ...
          <div style={{ padding: 20 }}>
            ...
            {students.map((s) => (
              <div key={s.id} style={{ marginTop: 20 }}>
                <h3>{s.name} (Grade: {s.grade})</h3>
                <button onClick={() => handlePredict(s.subjects)}>
